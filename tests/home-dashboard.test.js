...
test('buildNutritionTargetState：蛋白目标需要天然和特殊系数都设置', () => {
  ...
  assert.equal(state.activeGoal.pct, 62.5);
  assert.equal(state.activeGoal.pctText, '62.5%');
  assert.equal(state.activeGoal.status, 'pending');
  assert.equal(state.activeGoal.statusText, '还差 37.5%');
  assert.equal(state.activeGoal.ringPct, 62.5);
  assert.equal(state.activeGoal.overPct, 0);
  assert.equal(state.activeGoal.actualText, '已摄入 5.00g');
  assert.equal(state.activeGoal.goalText, '目标 8.00g');
  ...
  assert.equal(state.rows[0].targetText, '4.80g');
  assert.equal(state.rows[0].remainingText, '1.80g');
  assert.equal(state.rows[0].remainingLabel, '还差');
  assert.equal(state.rows[0].pctText, '62.5%');
  assert.equal(state.rows[1].targetText, '3.20g');
  assert.equal(state.rows[1].remainingText, '1.20g');
});
...
test('buildNutritionTargetState：热量目标只需要热量系数并按首选维度展示', () => {
  ...
  assert.equal(calorieOnly.activeGoal.pct, 83.3);
  assert.equal(calorieOnly.activeGoal.pctText, '83.3%');
  ...
  assert.equal(calorieOnly.rows[0].pctText, '83.3%');
...
test('buildNutritionTargetState：超出目标时给出超出状态和超出量', () => {
  ...
  assert.equal(state.activeGoal.statusText, '超出 50.0%');
  ...
  assert.equal(state.rows[0].statusText, '超出 50.0%');
});

test('buildNutritionSummary：只算实际摄入，热量取整、蛋白2位、含总蛋白与系数', () => {
  ...
  assert.equal(s.naturalProtein, 5.22);     // 2 位小数
  assert.equal(s.naturalProteinText, '5.22');
  assert.equal(s.specialProtein, 9.6);
  assert.equal(s.specialProteinText, '9.60');
  assert.equal(s.totalProtein, 14.82);      // 5.22+9.6=14.82
  assert.equal(s.totalProteinText, '14.82');
  assert.equal(s.naturalProteinPerKg, 1.2); // 5.22/4.35=1.2，2 位
  assert.equal(s.naturalProteinPerKgText, '1.20');
...
test('buildTimeline：合并各类记录并按时间倒序', () => {
  ...
  assert.equal(timeline[0].desc, '热量 80kcal · 天然蛋白 0.60g · 特殊蛋白 1.20g');
...
test('buildTimeline：辅食摘要显示热量与天然/特殊蛋白，不显示具体食物名', () => {
  ...
  assert.equal(timeline[0].desc, '含 2种食物 · 热量 70kcal · 天然蛋白 1.00g · 特殊蛋白 0.50g');
