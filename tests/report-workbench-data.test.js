...
test('buildReportWorkbenchView ignores recipe provenance fields when summarizing nutrition windows', () => {
  const reports = [
    {
      _id: 'urine-current',
      reportType: 'urine_ms',
  ...
  const recipeFeedingRecords = baseFeedingRecords.map((record) => ({
    ...record,
    intakes: (record.intakes || []).map((intake, index) => ({
      ...intake,
      sourceType: 'recipe',
      recipeId: `recipe_${index + 1}`,
  ...
  const baseView = buildReportWorkbenchView({
    selectedReportId: 'urine-current',
    reports,
    feedingRecords: baseFeedingRecords,
    nutritionSettings: {},
    fallbackWeight: 5
  });

  const recipeView = buildReportWorkbenchView({
    selectedReportId: 'urine-current',
    reports,
    feedingRecords: recipeFeedingRecords,
    nutritionSettings: {},
    fallbackWeight: 5
  });

  assert.deepEqual(recipeView.nutritionWindows, baseView.nutritionWindows);
});
