...
test('getDailySummariesForRange rebuildMissing 受 maxRebuildDays 限制，避免长区间触发云请求风暴', async () => {
  const { db } = createDbMock({ dailySummaryData: [] });
  const { service, models, restore } = loadFreshService(db);
  const rebuiltDates = [];
  models.FeedingRecordV2Model.getRecordsByDate = async (babyUid, date) => {
  ...
  try {
    await service.getDailySummariesForRange('baby-1', '2026-05-01', '2026-05-31', {
      rebuildMissing: true,
      maxRebuildDays: 3,
      rebuildDelayMs: 0
    });
    assert.equal(rebuiltDates.length, 3);
    assert.deepEqual(rebuiltDates, ['2026-05-01', '2026-05-02', '2026-05-03']);
  } finally {
    restore();
  }
});
