...
4. 报告添加展示的模块中，每个指标后面都需要输入值及范围，自动判断超出范围值上升还是下降
   - 新建报告时会自动带出同类型上一份报告的参考范围（`ReportRepository.getHistoricalRanges`）
   - 血串联的 C3/C0、C3/C2 比值指标会在填完 C3/C0/C2 后自动算出，无需手填
   - 支持"拍照识别填指标"：拍下报告单，通过微信云开发的通用印刷体识别自动填入数值和参考范围，识别到的字段会标记"识别待核对"，用户编辑后自动取消标记；保存时如果还有未核对的识别结果会二次确认。识别结果仅供参考，最终数值以用户核对后的为准
