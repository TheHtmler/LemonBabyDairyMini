...
> **2026-07-23 修订（方案 B）**  
> 食谱管理只维护原料组合，不填配方用量/成品总重。  
> 记本顿时选食谱 → 填本次各原料用量 → 汇总本次总重与总营养 → 再填宝宝摄入百分比或克数。
...
缺口是「成品菜」：例如番茄炒蛋，需要复用一组原料；但每次做菜用量可能不同，因此模板只记「有哪些原料」，当次用量与摄入比例在记本顿时填写。
...
- 创建食谱：从 `food_catalog` 选原料组合（不填用量）
- 记本顿：分流选「食谱」→ 填本次各原料用量 → 算本次总重/总营养 → 填摄入 **百分比或克数**
