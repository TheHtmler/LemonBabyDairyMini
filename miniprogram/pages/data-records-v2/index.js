...
function buildBowelStats(records = []) {
  return (records || []).reduce((stats, record = {}) => {
    const type = record.type || '';
    if (type === 'stool' || type === 'urine' || type === 'mixed') {
      stats[type] += 1;
      stats.total += 1;
    }
    return stats;
  }, createEmptyBowelStats());
}
...
function getBowelConsistencyIcon(consistency) {
  const icons = {
    liquid: '💧',
    loose: '🌊',
    soft: '🥣',
...
function getBowelSmellIcon(smell) {
  const icons = {
    normal: '🌿',
    sour: '🍋',
    fishy: '🐟',
...
function getUrineAmountIcon(amount) {
  const icons = {
    small: '💧',
    medium: '💦',
    large: '🌊'
...
function getUrineColorIcon(color) {
  const icons = {
    clear: '⚪',
    light_yellow: '🟡',
    dark_yellow: '🟠'
...
function getFormulaMilkPowder(feeding, nutritionSettings = {}) {
  return feedingCalculator.getFormulaMilkPowder(feeding, nutritionSettings);
}
...
function calculateMilkNutrition(feeding, nutritionSettings = {}) {
  if (feeding?.isV2FeedingRecord && feeding.nutritionSummary) {
    ...
    const calories = roundCalories(Number(summary.calories) || 0);
    return {
      naturalProtein: roundNumber(Number(summary.naturalProtein) || 0, 2),
      specialProtein: roundNumber(Number(summary.specialProtein) || 0, 2),
      protein: roundNumber(Number(summary.protein) || 0, 2),
      calories,
      naturalCalories: calories,
      specialCalories: 0,
      fat: roundNumber(Number(summary.fat) || 0, 2),
      carbs: roundNumber(Number(summary.carbs) || 0, 2)
    };
  }
  ...
  const formulaPowderWeight = getFormulaMilkPowder(feeding, nutritionSettings);
  ...
  if (naturalMilkType === 'formula') {
    const powderWeight = formulaPowderWeight || calculateFormulaPowder(naturalMilkVolume, nutritionSettings);
...
function calculateIntakeOverview(feedings = [], intakes = [], weight = 0, nutritionSettings = {}, treatmentRecords = []) {
  ...
  const milkTotals = aggregateMilkComponentTotals(legacyFeedings, nutritionSettings);
  ...
  return intakes
    .map(item => {
      ...
      const normalizedItem = {
        ...
        sourceType: item.sourceType || 'manual_food',
...
function foodIntakeRecordToLegacyIntake(record = {}) {
  ...
  const proteinSourceResolved = foodSnapshot.proteinSource || record.proteinSource || 'natural';
  // 兼容两种食物结构：v2 food_intake_records 的拆分在 nutrition 内；
  // v1 feeding_records.intakes 的拆分在顶层。都缺失时按 proteinSource 兜底，
  // 否则 v1 食物的天然/特殊蛋白会被清零，导致蛋白总计与系数漏算食物。
  const isNum = (value) => typeof value === 'number' && Number.isFinite(value);
  const proteinTotal = Number(nutrition.protein) || 0;
  const naturalProteinResolved = isNum(nutrition.naturalProtein)
    ? nutrition.naturalProtein
    : (isNum(record.naturalProtein)
      ? record.naturalProtein
      : (proteinSourceResolved === 'special' ? 0 : proteinTotal));
  const specialProteinResolved = isNum(nutrition.specialProtein)
    ? nutrition.specialProtein
    : (isNum(record.specialProtein)
      ? record.specialProtein
      : (proteinSourceResolved === 'special' ? proteinTotal : 0));
  const intake = {
...
function createDataRecordsPageConfig(options = {}) {
  return {
  recordSource: options.recordSource || 'legacy',
  data: {
    isV2RecordsPage: options.recordSource === 'v2',
    selectedDate: '',
  ...
  navigateToMealEditor() {
    ...
    wx.navigateTo({
      url: `/pkg-records/meal-editor/index?date=${selectedDate}&from=records`
    });
  },

  getMilkFeedingEditorPath() {
    return '/pkg-milk/milk-feeding-editor-v2/index';
  },
  ...
  navigateToMilkFeedingEditor() {
    ...
    wx.navigateTo({
      url: `${editorPath}?mode=create&source=records&date=${selectedDate}`
    });
  },
  ...
  navigateToTreatmentRecord() {
    const selectedDate = this.data.selectedDate || this.formatDate(new Date());
    wx.navigateTo({
      url: `/pkg-records/treatment-record/index?date=${selectedDate}`
    });
  },
  ...
  openEditTreatmentRecord(e) {
    ...
    if (!recordId) return;
    wx.navigateTo({
      url: `/pkg-records/treatment-record/index?id=${encodeURIComponent(recordId)}&date=${selectedDate}`
    });
  },
  ...
  navigateToBowelRecord() {
    const selectedDate = this.data.selectedDate || this.formatDate(new Date());
    wx.navigateTo({
      url: `/pkg-records/bowel-record/index?date=${selectedDate}`
    });
  },

  openEditBowelRecord(e) {
    const recordId = e.currentTarget?.dataset?.recordId || e.detail?.recordId;
    const selectedDate = this.data.selectedDate || this.formatDate(new Date());
    if (!recordId) return;
    wx.navigateTo({
  ...
  legacyIntakeKey(intake = {}) {
    ...
    return [
      intake.mealBatchId || '',
      intake.foodId || '',
      intake.recordedAt || intake.mealTime || '',
      intake.quantity || '',
      intake.nameSnapshot || ''
    ].join('|');
  },
  ...
  async deleteLegacyFoodIntakes(targets = []) {
    ...
    (targets || []).forEach(item => {
      const docId = item && item.legacyRecordId;
      if (!docId) return;
    ...
    if (byDoc.size === 0) {
      throw new Error('未找到食物所属的旧记录');
    }

    for (const [docId, items] of byDoc.entries()) {
      const res = await db.collection('feeding_records').doc(docId).get();
      const record = res.data || {};
      const intakes = Array.isArray(record.intakes) ? record.intakes : [];
      const removeKeys = new Set(items.map(item => this.legacyIntakeKey(item)));
      const remaining = intakes.filter(intake => !removeKeys.has(this.legacyIntakeKey(intake)));
      await db.collection('feeding_records').doc(docId).update({
        data: {
          intakes: remaining,
          updatedAt: db.serverDate()
        }
  ...
  async loadV2FeedingResult(babyUid, dateStr) {
    ...
    return {
      data: [createDailyRecordFromV2(records, dateStr, babyUid)]
    };
  },

  async loadLegacyDailyRecordResult(db, dateRangeQuery, dateStr, babyUid) {
    let result = await db.collection('feeding_records').where(dateRangeQuery).get();
    if (!result.data || result.data.length === 0) {
      result = await db.collection('feeding_records').where({ date: dateStr, babyUid }).get();
    }
    return result;
  },

  async mergeV2FeedingWithLegacySecondaryData(feedingResult, legacyResult, dateStr, babyUid) {
    const v2Records = feedingResult.data || [];
    const legacyRecords = legacyResult?.data || [];
    if (v2Records.length === 0 && legacyRecords.length === 0) {
      return { data: [] };
    }
    ...
    const legacyDailyRecord = legacyRecords.length > 0
      ? await this.consolidateFeedingRecords(legacyRecords)
      : null;
    ...
    return {
      data: [
        {
          ...v2DailyRecord,
          _id: legacyDailyRecord?._id || v2DailyRecord._id,
  ...
  async loadBasicInfoSnapshotForRecords(dateStr, babyUid) {
    ...
    const snapshot = await FeedingRecordV2Model.resolveBasicInfoSnapshot(babyUid, dateStr, {
  ...
  async fetchDailyRecords(dateStr, options = {}) {
    ...
    try {
      const useV2Records = this.isV2RecordsSource();

      // 确保配奶设置已加载；v2 记录已带营养快照，不读取旧配奶设置回退。
      if (!useV2Records && !this.data.nutritionSettings) {
        await this.loadNutritionSettings();
      }
  ...
  processFeedingData: function(feedingRecords, intakes = this.data.intakes || []) {
    ...
    const processedRecords = feedingRecords.map(record => {
      ...
      const nutrition = record?.isV2FeedingRecord && record.nutritionSummary
        ? calculateMilkNutrition(record, nutritionSettings)
        : calculateMilkNutrition({
            naturalMilkVolume: record.naturalMilkVolume,
            formulaPowderWeight: getFormulaMilkPowder(record, nutritionSettings),
            specialMilkVolume: feedingCalculator.getSpecialMilkVolume(record),
            specialMilkPowder: feedingCalculator.getSpecialMilkPowder(record, nutritionSettings),
  ...
  buildFeedingDisplay(record) {
    ...
    const specialMilkVolume = feedingCalculator.getSpecialMilkVolume(record);
    const nutrition = calculateMilkNutrition({
      naturalMilkVolume,
      formulaPowderWeight: getFormulaMilkPowder(record, settings),
      specialMilkVolume,
    ...
    const formulaPowderWeight = record.naturalMilkType === 'formula'
      ? getFormulaMilkPowder(record, settings)
      : 0;
  ...
  calculateTotalMilk: function(records) {
    ...
    records.forEach(record => {
      if (record?.isV2FeedingRecord && record.totalMilkVolume !== undefined) {
        totalNaturalMilk += parseFloat(record.totalMilkVolume || 0);
        return;
      }
  ...
  deleteFeeding(e) {
    ...
    this.setData({
      ...
      deleteType: 'feeding',
      deleteIndex: index,
      deleteRecordId: feeding._id || feeding.id || ''
    });
  },
  ...
  async confirmDelete() {
    ...
    try {
      if (deleteType === 'feeding') {
        if (this.isV2RecordsSource()) {
          const feeding = (this.data.feedingRecords || [])[deleteIndex] || {};
          const v2RecordId = deleteRecordId || feeding._id || feeding.id;
          if (!v2RecordId) {
  ...
  openEditFeedingModal(e) {
    ...
    const specialMilkPowder = feedingCalculator.getSpecialMilkPowder(
      feeding,
    ...
    try {
      ...
      if (targetIndex >= 0) {
        ...
        feedings[targetIndex] = {
          ...
          formulaPowderWeight: editingFeeding.naturalMilkType === 'formula'
            ? feedingCalculator.getFormulaMilkPowder(editingFeeding, this.data.nutritionSettings || {})
            : 0,
          specialMilkPowder: feedingCalculator.getSpecialMilkPowder(editingFeeding, this.data.nutritionSettings || {}),
  ...
  processBowelData(bowelRecords) {
    ...
    const processedRecords = bowelRecords.map(record => ({
      ...record,
      typeIcon: getBowelTypeIcon(record.type),
      consistencyIcon: getBowelConsistencyIcon(record.consistency),
      smellIcon: getBowelSmellIcon(record.smell),
      urineAmountText: getUrineAmountText(record.urineAmount),
      urineAmountIcon: getUrineAmountIcon(record.urineAmount),
      urineColorText: getUrineColorText(record.urineColor),
      urineColorIcon: getUrineColorIcon(record.urineColor),
    ...
    this.setData({
      ...
      bowelStats: buildBowelStats(processedRecords)
    });
  },
  ...
  buildV2PersistedBasicInfoForDate(updates = {}) {
    const pickNumberLike = (key) => {
      ...
      return this.isNumberValue(value) ? Number(value) : value;
    };
    ...
    return {
      ...
      specialProteinCoefficient: Object.prototype.hasOwnProperty.call(updates, 'specialProteinCoefficient')
        ? updates.specialProteinCoefficient
        : (this.data.specialProteinCoefficientInput || ''),
      calorieCoefficient: Object.prototype.hasOwnProperty.call(updates, 'calorieCoefficient')
        ? updates.calorieCoefficient
        : (this.data.calorieCoefficient || ''),
      ...updates
    };
  },
  ...
  async saveBasicInfoFields(updates = {}) {
    ...
    const db = wx.cloud.database();
    if (this.isV2RecordsSource()) {
      const basicInfo = this.buildV2PersistedBasicInfoForDate(updates);
      const result = await FeedingRecordV2Model.upsertGrowthRecordForDate(babyUid, selectedDate, basicInfo);
